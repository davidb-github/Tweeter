...
const tweetConcat = (concept, adjective) => {
    const string = " Learning " + concept + " in class today. It's really " + adjective;
...
const tweetInterp = (concept, adjective) => {
    const string = `
    Learning ${concept} in class today. It's really ${adjective}
    `
    return string;
...
tweet2 = tweetInterp("tacos", "delicious")
console.log(tweet2);
...
*/
// debugger
const tweetCard = (name, handle, msg) => {
    let htmlRep = `
    <article class="tweet">
        <div class="tweet__header">
            ${name}<span class="tweet-handle">${handle} </span>
        </div>
        <div class="tweet__message">${msg} </div>
    </article>
    `
    // get reference to DOM element
    const targetElement = document.querySelector("#tweet__container")

    // append HTML String to DOM element
    targetElement.innerHTML += htmlRep

};
tweetCard("david", "@Handle", tweet1)
tweetCard("david", "@Handle", tweet2)
// console.log(stuff)
